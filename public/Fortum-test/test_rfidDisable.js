...
//console.log(key_number.toString());

//var key_number = "80000000064cb7";
//var key_number = keynumberReceive.toString();

var lastChargingKey_txt = "";

// read txt last_charge.txt file
if (!fs.existsSync('./last_chargeKey.txt')){
fs.writeFile('last_chargeKey.txt', '', function (err) {
  console.log('last_chargeKey.txt file created because it is not found.');
});
}

try{
lastChargingKey_txt = fs.readFileSync('last_chargeKey.txt').toString();
lastChargingKey_txt = lastChargingKey_txt.replace(/\s/g, '');
console.log(lastChargingKey_txt);
}
catch(err){
console.log(err);
}
// Check for other running test process
var a = 0;
var b = 1;
var runnrunningStatus = "";

while(a<b) {
runningStatus = fs.readFileSync('running_status.txt').toString();
runningStatus = runningStatus.replace(/\s/g, '');
if(runningStatus == "false") {
++a;
console.log("Test is starting ...");
}
//console.log("Other test is running ...");
++a;
++b;
}
...
fs.writeFile('running_status.txt', 'true', function (err) {
...
try{
    ...
    if (enabled == "No"){
        enabled = false;
        sleep(4);
        var enableCheck_log = "CHARGING KEY DISABLED. NOT EDIT.";
    } else{
    var enableCheck_log = "CHARGING KEY ENABLED. CONTINUE";
    }
  }
  catch (err){
  //console.log(err);
...
test_log = dir + "/" + test_log + ".txt";
  var log_result = "--TEST LOGS--\n\n";
    ...
    // read txt last_charge.txt file
  fs.writeFile('last_chargeKey.txt', keyNumberDisabled, function (err) {
    if (err) throw err;
    console.log('last_chargeKey.txt file created.');
  });

  fs.writeFile('enableKey.txt', enabledKey, function (err) {
    if (err) throw err;
    console.log('enableKey.txt file created.');
  });
  }
  else if (status == 1){
    // Create a log file with test results
...
// Update status process
fs.writeFile('running_status.txt', 'false', function (err) {
  console.log('running_status.txt updated');
});
}
