...
class MailEclipse
{
    ...
    public static function createTemplate($request)
    {
        ...
        if (! view()->exists($view) && ! self::getTemplates()->contains('template_slug', '=', $templateName)) {
            ...
            file_put_contents($dir."/{$templateName}.blade.php", self::templateComponentReplace($request->template_editor));

            file_put_contents($dir."/{$templateName}_plain_text.blade.php", $request->plain_text);

            return redirect()->route('templateList')->with('success_message',  trans("translate.template_was_successfully_added"));
        }
        
        return redirect()->route('templateList')->with('error_message', trans("translate.unexpected_error"));
